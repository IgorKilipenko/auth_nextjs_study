...
const GoogleColoredIcon = ({ fill, ...restProps }: IconProps) => {
    return (
        <Icon viewBox="0 0 25 25" fill={fill} {...restProps}>
            <g
                id="Group_69890"
...
const GoogleColoredIconV2 = (props: HoveredIconProps) => {
    return (
        <Icon viewBox="0 0 48 48" {...props}>
            <g transform-origin="50% 50%" transform="scale(1.12)">
                <path
                    fill="#fbc02d"
...
const social = {
    facebook: ({ hovered = false, ...restProps }: HoveredIconProps) => (
        <Icon
            as={FaFacebook}
            fill={hovered ? 'blue.600' : 'blue.700'}
            {...restProps}
        />
    ),
    google: ({ hovered = false, ...restProps }: HoveredIconProps) => (
        <Icon
            as={FaGoogle}
            fill={hovered ? 'yellow.300' : 'yellow.400'}
            {...restProps}
        />
    ),
    googleColored: (props: HoveredIconProps) => (
        <GoogleColoredIcon {...props} />
    ),
    googleColoredV2: ({ hovered = false, ...restProps }: HoveredIconProps) => (
        <GoogleColoredIconV2
            {...(hovered && { stroke: 'gray.50' })}
            {...restProps}
        />
    ),
}
...
export interface HoveredIconProps extends IconProps {
    hovered?: boolean
}
